...
/** Splits on `,` like a CSV row, except commas wrapped in `'...'` are kept literal (quotes are stripped). */
function splitCells(str: string): string[] {
  const out: string[] = [];
  let cur = "";
  let inQuotes = false;
  for (const ch of str) {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (type === "table") {
      const segs = rest
        ...
        .map((s) => s.trim())
        .filter(Boolean);
      const cols = splitCells(segs[0] || "").filter(Boolean);
      ...
      if (fs[1]) {
        ...
        if (kind === "currency" || kind === "moeda")
          formulaFormat = {
            kind: "currency",
            currency: (fp[1] || "BRL").toUpperCase(),
          };
        else if (kind === "int" || kind === "inteiro")
          formulaFormat = { kind: "int" };
      }
    } else if (rest) {
      options = rest
        .split(",")
...
export class MarkerEngine {
  ...
  linkValues(): Record<string, unknown> {
    ...
    this.getFields().forEach((info) => {
      if (info.link && !info.linkValue)
        map[info.link] = this.values[info.slug] || "";
    });
  ...
  isVisible(info: FieldInfo): boolean {
    ...
    return (
      ...
      String(info.linkValue).trim().toLowerCase()
    );
  }
  ...
  private evalCondition(expr: string): boolean {
    ...
    if (!m) {
      const cur = String(this.condValue(slug(expr.trim())) || "")
        .trim()
        .toLowerCase();
      return !!cur && cur !== "false" && cur !== "não" && cur !== "nao";
    }
    const cur = String(this.condValue(slug(m[1].trim())) || "")
      .trim()
      .toLowerCase();
  ...
  private sumTableColumn(label: string, column: string): number {
    const info = this.getFields().find(
      (f) => f.slug === slug(label) && f.type === "table",
    );
    if (!info) return 0;
    const rows = Array.isArray(this.values[info.slug])
      ? (this.values[info.slug] as string[][])
      : info.presetRows || [];
    const ci = info.options.findIndex(
  ...
  evalFormula(info: FieldInfo, depth = 0): number {
    ...
    expr = expr.replace(/sum\(\s*([^.)]+)\.([^)]+)\)/gi, (_mm, lbl, col) =>
      String(this.sumTableColumn(lbl, col)),
    );
    expr = expr.replace(/\[([^\]]+)\]/g, (_mm, lbl) => {
      const slugKey = slug(lbl);
      const ref = this.getFields().find((f) => f.slug === slugKey);
      const num =
        ref && ref.type === "formula"
          ? this.evalFormula(ref, depth + 1)
          : parseNum(this.values[slugKey]);
  ...
  formulaText(info: FieldInfo): string {
    ...
    if (fmt && fmt.kind === "currency")
      return formatCurrency(String(r), fmt.currency || "BRL");
    if (fmt && fmt.kind === "int")
      return new Intl.NumberFormat("pt-BR", {
  ...
  displayValue(info: FieldInfo, val: unknown): string {
    ...
    if (info.type === "currency")
      return formatCurrency(val, (info.options && info.options[0]) || "BRL");
    if (info.type === "yesno")
