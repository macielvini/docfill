...
export function docScreen(props: DocScreenProps): TemplateResult {
  ...
  return html`
      ...
      ${segmentedToggle([
        {
          label: "1 · Preencher",
          active: step === "fill",
          onClick: () => props.onStepChange("fill"),
        },
        {
          label: "2 · Revisar",
          active: step === "review",
      ...
      ? html`<div class="flex flex-col gap-4 px-4.5 pt-4 pb-32.5">
          <div class="flex flex-col gap-2">
            <div class="flex items-baseline justify-between">
              <span class="text-xs font-semibold"
                >${props.filledCount} de ${props.total} preenchidos</span
              >
          ...
          ${repeat(
            ...
            (it) =>
              it.kind === "heading"
                ? sectionHeading(it)
                : fieldEditor(it.field, props.sigHost),
          )}
        </div>`
      : nothing}
    ${step === "review"
      ? html`<div class="flex flex-col gap-3.5 px-4.5 pt-4 pb-32.5">
          <div class="card card-border bg-base-100 shadow-sm">
            <div class="card-body p-7 bg-gray-50">
              ${renderDoc(props.docText, props.values, false)}
            </div>
          </div>
          ${props.signFields.length > 0
            ? html`<div class="flex gap-2.5">
                <button
                  ...
                  @click=${props.onOpenSignModal}
                >
                  Assinar
                </button>
                <button
                  class="btn btn-outline flex-none"
                  @click=${props.onRequestClearSignatures}
                >
                  Limpar assinaturas
                </button>
              </div>`
    ...
    ${props.signModalIndex !== null
      ? (() => {
          const advance =
            props.signModalIndex === props.signFields.length - 1
              ? props.onCancelSignModal
              : props.onAdvanceSignModal;
          return signModal({
            ...
            onAdvance: advance,
            onSkip: () => {
              props.sigHost.setValue(
                props.signFields[props.signModalIndex!].slug,
                "",
              );
              advance();
            },
    ...
    ${step === "review"
      ? html`<div class="flex justify-end px-4.5">
          <label class="label cursor-pointer gap-1.5 text-xs">
            <input
              ...
              .checked=${props.printCompact}
              @change=${props.onTogglePrintCompact}
            />
      ...
      : nothing}
    <div
      class="sticky bottom-0 z-20 flex gap-2.5 bg-linear-to-t from-base-200 from-62% to-transparent px-4.5 pt-3.5 pb-4"
    >
      ${props.canBack
        ? html`<button class="btn btn-outline flex-none" @click=${props.onBack}>
            Voltar
          </button>`
        : nothing}
      ${step === "review"
        ? html`<button
            class="btn btn-outline flex-1 hidden"
            @click=${props.onEnviarParaRevisao}
          >
            Enviar para revisão
          </button>`
        : nothing}
      ${step === "review"
        ? html`<button
            class="btn btn-outline btn-square flex-none"
            @click=${props.onPrint}
            title="Imprimir"
          >
            ${icon("printer")}
      ...
      ${step === "review"
        ...
        : nothing}
      <button class="btn btn-primary flex-1" @click=${props.onNext}>
        ${props.nextLabel}
      </button>
    </div>
  `;
